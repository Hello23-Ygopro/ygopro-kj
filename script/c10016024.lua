...
--return
function scard.thfilter(c,pwr)
	return c:IsFaceup() and c:IsPowerBelow(pwr) and c:IsAbleToHand()
end
function scard.tg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local pwr=(Duel.GetTurnPlayer()==tp and 8000 or 4000)
	if chkc then return chkc:IsLocation(LOCATION_BZONE) and scard.thfilter(chkc,pwr) end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	Duel.SelectTarget(tp,scard.thfilter,tp,LOCATION_BZONE,LOCATION_BZONE,1,1,nil,pwr)
end
